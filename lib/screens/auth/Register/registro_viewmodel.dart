...
import 'package:andromeda/screens/auth/Register/registro_state.dart';
  ...
  changeUsername(String value){
    if (value.length >= 5){
      _state = _state.copyWith(username: ValidationItem(value: value, error: ''));
       ...
       _state = _state.copyWith(username: ValidationItem(error: 'Al menos 8 caracteres' ));
    }
    notifyListeners();
  }
   ...
   changePassword(String value){
    if (value.length >= 5){
      _state = _state.copyWith(password: ValidationItem(value: value, error: ''));
          ...
          _state = _state.copyWith(password: ValidationItem(error: 'Al menos 8 caracteres' ));
    }
        notifyListeners();
  }
  ...
  changeconfirmPassword(String value){
    if (value.length >= 5){
      _state = _state.copyWith(confirmPassword: ValidationItem(value: value, error: ''));
          ...
          _state = _state.copyWith(confirmPassword: ValidationItem(error: 'Al menos 8 caracteres' ));
